 ...
 */
function Client(baseUrl, user, pass, backoffArgs) {
  var self = this;
  backoffArgs = backoffArgs || { delay: 100 };
...
Client.prototype.start = function (apps, subscribeAll, callback) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var retry = backoff.create( self._connection.backoffArgs );
    ...
    function processOpen () {
      ...
      retry = backoff.create( self._connection.backoffArgs );
 ...
 *  @param {string} user - The username for the ARI instance
 *  @param {string} pass - The password for the ARI instance
 *  @param {Object} backoffs - Backoff functions for reconnecting
 *  @param {Client-clientCallback} cb -
 *    The callback to be called upon connection
 *  @returns {Q} promise - a promise that will resolve to a client
 */
module.exports.connect = function (baseUrl, user, pass, backoffArgs
    /**
  ...
  if ( typeof backoffArgs === 'object' ) {
    var client = new Client(baseUrl, user, pass, backoffArgs);
  } else {
